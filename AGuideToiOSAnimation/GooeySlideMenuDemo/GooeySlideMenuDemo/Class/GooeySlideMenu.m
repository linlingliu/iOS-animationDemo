...
-(void)addButtons:(NSArray *)titles{
    if (titles.count % 2 == 0) {
        ...
        for (NSInteger i = 0; i < titles.count; i++) {
            ...
            SlideMemuButton *home_button = [[SlideMemuButton alloc]initWithTitle:title];
            if (i >= titles.count / 2) {
                index_up ++;
...
-(void)trigger{
    if (!triggered) {
        ...
        [UIView animateWithDuration:50 delay:0.0f usingSpringWithDamping:1.0f initialSpringVelocity:0.9f options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction animations:^{
        ...
        [self beforeAnimation];
        [UIView animateWithDuration:50 delay:0.0f usingSpringWithDamping:8.0f initialSpringVelocity:2.0f options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionAllowUserInteraction animations:^{
...
-(void)displayLinkAction:(CADisplayLink *)dis{
    CALayer * side=(CALayer *)[helperSideView.layer presentationLayer];
    CALayer * center=(CALayer*)[helperCenterView.layer presentationLayer];
    CGRect sideRect=[[side valueForKey:@"frame"] CGRectValue];
    CGRect centerRect=[[center valueForKey:@"frame"]CGRectValue];
